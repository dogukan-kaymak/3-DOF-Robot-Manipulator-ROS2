...
int ansi_color_pair(short foreground, short background) {
  ...
  const int key = ((static_cast<int>(foreground) + 2) << 8) |
  ...
  const int next_pair =
      kAnsiColorPairBase + static_cast<int>(g_ansi_color_pairs.size());
...
void set_color_role(Theme &theme, int role, short foreground, short background,
                    int attributes = A_NORMAL) {
  theme[static_cast<std::size_t>(role)] =
      ThemeColor{foreground, background, attributes};
}
...
std::vector<HelpBlock> parse_help_blocks(const std::string &text) {
  ...
  while (start < text.size()) {
    ...
    std::string block_text = separator == std::string::npos
                                 ? text.substr(start)
    ...
    if (!block_text.empty()) {
      ...
      if (split == std::string::npos) {
        ...
        blocks.push_back(
            {block_text.substr(0, split), trim(block_text.substr(split + 1))});
      }
    }
...
void install_function_key_sequences() {
  ...
  static const Binding bindings[] = {
      {"\x1bOP", KEY_F(1)},    {"\x1bOQ", KEY_F(2)},    {"\x1bOR", KEY_F(3)},
      {"\x1bOS", KEY_F(4)},    {"\x1b[11~", KEY_F(1)},  {"\x1b[12~", KEY_F(2)},
      {"\x1b[13~", KEY_F(3)},  {"\x1b[14~", KEY_F(4)},  {"\x1b[15~", KEY_F(5)},
      {"\x1b[17~", KEY_F(6)},  {"\x1b[18~", KEY_F(7)},  {"\x1b[19~", KEY_F(8)},
      {"\x1b[20~", KEY_F(9)},  {"\x1b[21~", KEY_F(10)}, {"\x1b[23~", KEY_F(11)},
      {"\x1b[24~", KEY_F(12)},
  };
...
void apply_role_chgat(int row, int col, int count, int role,
...
int find_best_match(const std::vector<std::string> &labels,
                    const std::string &query, int current_index) {
  ...
  for (int pass = 0; pass < 2; ++pass) {
    for (int offset = 0; offset < count; ++offset) {
      ...
      const std::string haystack =
          to_lower(labels[static_cast<std::size_t>(index)]);
...
TerminalPane::TerminalPane() : current_style_(default_style_) {}

TerminalPane::~TerminalPane() { shutdown_process(); }

bool TerminalPane::visible() const { return visible_; }
...
void TerminalPane::hide() { visible_ = false; }
...
void TerminalPane::draw(int top, int left, int bottom, int right) {
  ...
  for (int row = 0; row < inner_rows; ++row) {
    for (int column = 0; column < inner_columns; ++column) {
      const auto &cell = screen_rows_[static_cast<std::size_t>(row)]
                                     [static_cast<std::size_t>(column)];
      attrset(color_attr_for_style(cell.style));
      mvaddch(top + 1 + row, left + 1 + column,
              static_cast<unsigned char>(cell.glyph));
    }
    attrset(A_NORMAL);
  }
...
void TerminalPane::spawn_process() {
  ...
  winsize size{};
  size.ws_row = static_cast<unsigned short>(std::max(1, LINES - 5));
  size.ws_col = static_cast<unsigned short>(std::max(1, COLS - 2));
...
void TerminalPane::reap_process() {
  ...
  if (WIFEXITED(status)) {
    append_status_message("Shell exited with code " +
                          std::to_string(WEXITSTATUS(status)) + ".");
  } else if (WIFSIGNALED(status)) {
    append_status_message("Shell terminated by signal " +
...
void TerminalPane::resize_screen_buffer(int rows, int columns) {
  ...
  const int old_columns =
      old_rows == 0 ? 0 : static_cast<int>(screen_rows_.front().size());
  std::vector<std::vector<TerminalCell>> resized(static_cast<std::size_t>(rows),
                                                 blank_row(columns));

  int new_cursor_row = cursor_row_;
  if (new_cursor_row >= rows) {
    new_cursor_row = rows - 1;
  }
  const int shift = cursor_row_ - new_cursor_row;

  for (int old_row = 0; old_row < old_rows; ++old_row) {
    const int new_row = old_row - shift;
    if (new_row >= 0 && new_row < rows) {
      const int cols_to_copy = std::min(old_columns, columns);
      for (int col = 0; col < cols_to_copy; ++col) {
        resized[static_cast<std::size_t>(new_row)][static_cast<std::size_t>(col)] =
            screen_rows_[static_cast<std::size_t>(old_row)][static_cast<std::size_t>(col)];
      }
    }
  }

  screen_rows_ = std::move(resized);
  cursor_row_ = new_cursor_row;
...
void TerminalPane::clear_cells(int row, int start_column, int end_column) {
  ...
  std::fill(target_row.begin() + start, target_row.begin() + end,
            default_cell());
}
...
std::vector<TerminalPane::TerminalCell>
TerminalPane::blank_row(int columns) const {
  return std::vector<TerminalCell>(static_cast<std::size_t>(columns),
                                   default_cell());
}
...
void TerminalPane::put_printable(char byte) {
  ...
  auto &cell = screen_rows_[static_cast<std::size_t>(cursor_row_)]
                           [static_cast<std::size_t>(cursor_column_)];
  cell.glyph = byte;
...
void TerminalPane::process_output_char(char byte) {
  ...
  if (byte == '\b' || byte == 127) {
    if (pending_wrap_) {
      pending_wrap_ = false;
    } else if (cursor_column_ > 0) {
      move_cursor(cursor_row_, cursor_column_ - 1);
    } else if (cursor_row_ > 0) {
      move_cursor(cursor_row_ - 1, std::max(0, last_columns_ - 1));
    }
    return;
  }
  if (byte == '\t') {
    const int next_stop =
        std::min(last_columns_ - 1, ((cursor_column_ / 8) + 1) * 8);
...
void TerminalPane::handle_csi_sequence(char final_byte,
  ...
  case 'f': {
    ...
    const int column =
        params.size() > 1 ? clamp_terminal_param(params[1], 1) : 1;
  ...
  case 'J': {
    ...
    if (mode == 2) {
      ...
      for (int row = cursor_row_ + 1;
           row < static_cast<int>(screen_rows_.size()); ++row) {
        clear_row(row);
      }
    }
    return;
  ...
  case 'P': {
    ...
    for (int column = cursor_column_; column < last_columns_; ++column) {
      ...
      row[static_cast<std::size_t>(column)] =
          source < last_columns_ ? row[static_cast<std::size_t>(source)]
                                 : default_cell();
    }
    return;
  }
  case '@': {
    ...
    for (int column = last_columns_ - 1; column >= cursor_column_; --column) {
      const int source = column - count;
      row[static_cast<std::size_t>(column)] =
          source >= cursor_column_ ? row[static_cast<std::size_t>(source)]
                                   : default_cell();
    }
    return;
  }
  case 'X':
    clear_cells(cursor_row_, cursor_column_,
                cursor_column_ + clamp_terminal_param(first, 1));
    return;
  case 'm':
    apply_sgr(params);
    return;
...
bool use_unicode_line_drawing() {
  ...
  return codeset != nullptr &&
         std::string(codeset).find("UTF-8") != std::string::npos;
}
...
void add_braille_dot(std::vector<uint8_t> &cells, int cell_columns,
                     int cell_rows, int virtual_column, int virtual_row) {
  if (cell_columns <= 0 || cell_rows <= 0 || virtual_column < 0 ||
      virtual_row < 0 || virtual_column >= cell_columns * 2 ||
      virtual_row >= cell_rows * 4) {
    return;
  }
  ...
  const auto cell_index =
      static_cast<std::size_t>(cell_row * cell_columns + cell_column);
...
std::string default_theme_config_path() {
  const std::string installed =
      ament_index_cpp::get_package_share_directory("ros2_console_tools") +
      "/config/tui_theme.yaml";
...
void draw_box_char(int row, int col, const cchar_t *wide_char,
                   char ascii_char) {
  if (use_unicode_line_drawing()) {
...
void draw_bar(int row, int columns, const std::string &text, int color_pair,
              int left_padding) {
  const auto &color = current_theme()[static_cast<std::size_t>(color_pair)];
  attron(COLOR_PAIR(color_pair) | color.attributes);
  mvhline(row, 0, ' ', columns);
  const int content_width = std::max(0, columns - left_padding);
  mvaddnstr(row, left_padding, truncate_text(text, content_width).c_str(),
            content_width);
  attroff(COLOR_PAIR(color_pair) | color.attributes);
}
...
void draw_status_bar(int row, int columns, const std::string &text) {
  ...
  mvhline(row, 0, ' ', columns);
  mvaddnstr(row, 1, truncate_text(text, std::max(0, columns - 1)).c_str(),
            std::max(0, columns - 1));
  attroff(COLOR_PAIR(kColorStatus) | color.attributes);
}

void draw_help_bar_region(int row, int left, int width,
...
void draw_search_box(int rows, int columns, const SearchState &state,
  ...
  mvaddnstr(top + 1, input_left,
            truncate_text(state.query, input_width).c_str(), input_width);

...
void TopicMonitorBackend::refresh_topics() {
...

  {
    ...
    // NOTE: Do NOT clamp selected_index_ here using topics_.size(), because that is
    // the raw topic count, not the visible items count. When namespaces are expanded,
    // visible items = topics + namespace headers, so topics_.size() - 1 is always too
    // small. Correct clamping happens in clamp_topic_selection() which uses the actual
    // visible_topic_items() list and is called before every draw and key operation.
